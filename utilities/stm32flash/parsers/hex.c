...
parser_err_t hex_open(void *storage, const char *filename, const char write) {
	...
	if (write) {
		...
		while(read(fd, &mark, 1) != 0) {
			if (mark == '\n' || mark == '\r') continue;//如果读到换行符或者回车符就跳过
			if (mark != ':')//每条记录必须以冒号开头
			...
			//2+4+2位16进制数;sscanf读取格式化输入，并返回成功读取到的数据的个数
			...
			switch(type) {
				/* data record */
				case 0:
					c = address - last_address;//检查地址间是不是有空隙
					...
					record = &st->data[st->data_len]; //更新记录地址
					st->data_len += reclen;
					break;
			...
			buffer[2] = 0;
			for(i = 0; i < reclen; ++i) {
				//从hex文件读取2个字符，并且将这两个字符以16进制形式输出
