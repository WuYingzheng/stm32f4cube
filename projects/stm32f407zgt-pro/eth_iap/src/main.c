...
int main(void)
{
  ...
  BSP_BUTTON_Init(BUTTON1, BUTTON_MODE_GPIO);
  BSP_LED_Init(LED0);  
  BSP_LED_Init(LED1);
  uartLogInit(USART1);
  
  /* Test if Key push-button is not pressed */
  if (!BSP_Button_Pressed(BUTTON1))
  { /* Key push-button not pressed: jump to user application */
    ...
    {
      /* LED１ (blue) ON to indicate bad software (when not valid stack address) */
      BSP_LED_On(LED1); // 非０表示错误
      while(1);
    }
  }
  /* Enter in IAP mode */
