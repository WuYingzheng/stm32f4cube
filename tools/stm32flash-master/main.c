...
int main(int argc, char* argv[]) {
	...
	if (action == ACT_WRITE) {
		/* first try hex 首先尝试hex格式*/
		if (!force_binary) {
			parser = &PARSER_HEX;//将对象“PARSER_HEX”对象的地址到parser
			p_st = parser->init();//调用parser_hex的初始化函数，为hex_t结构体分配内存空间，初始化为0,返回内存的地址
			if (!p_st) {
	...
	//尝试打开设备端口，目前支持I2C，USART，将 port_interface 结构体类型的赋给port对象（i2c接口或者usart接口）
	//这之后将会注册端口结构体
...
void show_help(char *name) {
	fprintf(stderr,
		...
		"   For stm32f407 develop-board, entry below:\n"
