...
//打开串口，为serial_t结构体申请内存并初始化为0,打开后给串口加锁
static serial_t *serial_open(const char *device)
{
	...
	fcntl(h->fd, F_SETFL, 0); //设置文件描述符的状态标志。

	tcgetattr(h->fd, &h->oldtio); //获取与终端相关的参数
	tcgetattr(h->fd, &h->newtio);

	return h;
}

static void serial_flush(const serial_t *h)//清除收到的数据
{
	tcflush(h->fd, TCIFLUSH); //清除正收到的数据，且不会读取出来
}
...
static void serial_close(serial_t *h)
{
	...
	lockf(h->fd, F_ULOCK, 0);  //解锁串口
	close(h->fd);              //关闭文件
	free(h);                   //释放控制结构体内存
}

//初始化私有数据结构serial_t *h，并且设置串口的属性
			       ...
			       const serial_stopbit_t stopbit)
{
	...
	cfmakeraw(&h->newtio);  //将终端设置为原始模式，不能回显...
#else /* __sun */
	...
	/* set the settings */
	serial_flush(h);
	if (tcsetattr(h->fd, TCSANOW, &h->newtio) != 0)//改变终端属性，TCSANOW为立刻改变
		return PORT_ERR_UNKNOWN;
...
//下面的2个函数都是调用上面的函数，做了简单的封装。
//在port.c中，open函数还用来检查该接口是否可用
				     ...
				     size_t nbyte)
{
	...
	while (nbyte) {
		r = read(h->fd, pos, nbyte);//读取n字节到pos所指的地址中
		if (r == 0)
